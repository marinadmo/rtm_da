...
def compute_dal(date, tpd_dir, json_dir) :    
    ...
    for i in range(-(interval - 1), 1):
        ...
        dyn_algos_return = read_dyn_algos(sat_id, f"{json_dir}{current_date.strftime('%Y')}/{current_date.strftime('%m')}/", corr, period, current_date, centre, algorithms, ignore, yesterday)
        # TPD files
...
def compute_coeffs(date, dal_norm, list_tpd_files, coeffs_filename) :
    ...
    with open(coeffs_filename, 'w', newline = '') as file:
        ...
        for ich, ch in enumerate(globalVars.channels):
            row = [date if ich == 0 else '', ch, m_a1[ich], m_a2[ich], m_c[ich]]
            writer.writerow(row)  
...
def read_csv_coefficients_plan(coeffs_filename, date) :
    ...
    for ich, ch in enumerate(globalVars.channels) :
        ...
        coeffs[ch]['a1'] = df.iloc[index[0] + ich]['a1']
        m_a2.append(df.iloc[index[0] + ich]['a2'])
        coeffs[ch]['a2'] = df.iloc[index[0] + ich]['a2']
        m_c.append(df.iloc[index[0] + ich]['c'])
        coeffs[ch]['c'] = df.iloc[index[0] + ich]['c']
...
def run_rtm(date, sat_dir, model_dir, days_forecast = 10, version = 0, coeffs_filename = None):
    ...
    # Open satellite file    
    files_sat = glob.glob(os.path.join(sat_dir, date[0:4], f"*{date}*nc")) #sat_dir + date[0:4] + '/*' + date + '*nc')
    print(files_sat)
