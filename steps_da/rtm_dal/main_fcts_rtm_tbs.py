...
def compute_dal() : #date, tpd_dir, json_dir) :    
    ...
    for i in range(-(interval - 1), 1):
        ...
        # TPD files
        swath_filename = f"{config.tpd_data_dir}dyn_tp_nh_amsr_gw1_ucorr_{current_date_str}_oneday.nc" 
...
def run_rtm(version = 1) : #date, sat_dir, model_dir, days_forecast = 10, version = 0, coeffs_filename = None):
...
def save_rtm_tbs(tb_rtm, newfiles_dir) : #date, tb_rtm, newfiles_dir) :   
